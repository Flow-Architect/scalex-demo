        ...
        {phases.length > 0 ? (
          <ol className="mt-3 space-y-2">
            {phases.map((phase, index) => (
              <li className="flex gap-2 text-sm leading-5 text-zinc-700" key={`${phase}-${index}`}>
                <span className="flex h-5 w-5 flex-none items-center justify-center rounded-md bg-violet-100 text-xs font-semibold text-violet-900">
                  {index + 1}
                </span>
                <span>{phase}</span>
              </li>
        ...
        {proposedTools.length === 0 ? (
          ...
          <div className="flex flex-wrap gap-2">
            {proposedTools.map((toolName, index) => (
              <span
                className="rounded-md border border-violet-200 bg-violet-50 px-2 py-1 text-xs font-semibold text-violet-900"
                key={`${toolName}-${index}`}
              >
                #{index + 1} {toolName}
              </span>
      ...
      <InspectorSection title="Agent task list">
        {tasks.length === 0 ? (
          ...
          <div className="grid gap-2">
            {tasks.map((task) => (
              <div className="border border-zinc-200 bg-zinc-50 p-3" key={task.agent}>
                <p className="text-sm font-semibold text-zinc-950">{task.agent}</p>
                <p className="mt-1 text-sm leading-5 text-zinc-700">{task.task}</p>
              </div>
            ))}
          </div>
        )}
